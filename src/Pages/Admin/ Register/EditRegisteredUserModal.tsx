...
const EditRegisteredUserModal = ({ data, id }: any) => {
  const { edituserisSuccess, edituserisLoading } = useAppSelector(
    (state: any) => state.reg
  );
        ...
        <Modal.Body>
          <div className='page-btn-title'>
            {Edit?.map((catagory, i) => (
              <button
                id={
                  activeTab === i ? "custom-btn-two-active" : "custom-btn-two"
                }
                onClick={() => {
                  showInfo(catagory);
                  setActiveTab(i);
                  ...
                  <div className='user-status mb-5'>
                    ...
                    <label className='toggle-switch'>
                      <input
                        ...
                        onChange={(e) =>
                          handleOnChange("activated", e.target.checked)
                        }
                      />
          ...
          {result === "Reset Password" && <ResetPasswordModal id={data?.id} />}
        </Modal.Body>
      </Modal>
    </>
  );
